...
func runSyncTest(gt *testing.T, lightModeRPC string) {
	...
	if os.Getenv("CIRCLECI_PIPELINE_SCHEDULE_NAME") != "build_daily" && os.Getenv("CIRCLECI_PARAMETERS_SYNC_TEST_OP_NODE_DISPATCH") != "true" {
		t.Skipf("TestSyncTesterLightModeExtEL only runs on daily scheduled pipeline jobs: schedule=%s dispatch=%s", os.Getenv("CIRCLECI_PIPELINE_SCHEDULE_NAME"), os.Getenv("CIRCLECI_PARAMETERS_SYNC_TEST_OP_NODE_DISPATCH"))
	}

	l := t.Logger()
	require := t.Require()
	blocksToSync := uint64(20)
	sys, target := setupSystem(gt, t, blocksToSync, lightModeRPC)
	...
	require.GreaterOrEqual(stSession.CurrentState.Latest, stSession.InitialState.Latest+blocksToSync, "SyncTester session Latest should have synced target blocks")
	require.GreaterOrEqual(stSession.CurrentState.Safe, stSession.InitialState.Safe+blocksToSync, "SyncTester session Safe should have synced target blocks")

	modeStr := "CL derivation from L1"
	if lightModeRPC != "" {
		modeStr = fmt.Sprintf("light CL mode (RPC: %s)", lightModeRPC)
	}
	l.Info("SyncTester Light CL Mode ExtEL test completed successfully",
		"mode", modeStr,
		"l2cl_chain_id", sys.L2CL.ID().ChainID(),
		"l2cl_sync_status", l2CLSyncStatus)
}

// TestSyncTesterLightModeExtEL tests op-node syncing with or without light CL mode
func TestSyncTesterLightModeExtEL(gt *testing.T) {
	lightModeRPC := os.Getenv("OP_NODE_LIGHT_MODE_RPC")
	runSyncTest(gt, lightModeRPC)
}

// setupSystem initializes the system for the test and returns the system and the target block number of the session
func setupSystem(gt *testing.T, t devtest.T, blocksToSync uint64, lightModeRPC string) (*presets.MinimalExternalEL, uint64) {
	// Initialize orchestrator
	orch, target := setupOrchestrator(gt, t, blocksToSync, lightModeRPC)
...
func setupOrchestrator(gt *testing.T, t devtest.T, blocksToSync uint64, lightModeRPC string) (*sysgo.Orchestrator, uint64) {
	l := t.Logger()
	ctx := t.Ctx()
	require := t.Require()
	...
	l.Info("Runtime configuration values for TestSyncTesterLightModeExtEL")
	...
	l.Info("LIGHT_MODE_RPC", "value", lightModeRPC)
	...
	// Set light CL mode environment variable if provided
	// The op-node will automatically pick this up during initialization
	if lightModeRPC != "" {
		os.Setenv("OP_NODE_LIGHT_MODE", "true")
		os.Setenv("OP_NODE_LIGHT_MODE_RPC", lightModeRPC)
	}
