...
func TestSyncTesterLightMode(gt *testing.T) {
	t := devtest.SerialT(gt)
	sys := presets.NewSimpleWithSyncTesterLightMode(t)
	require := t.Require()
	logger := t.Logger()
	ctx := t.Ctx()
	...
	// Stop L2CL2 which is running in light CL mode
	sys.L2CL2.Stop()
	...
	// Restarting will allow L2CL2 to query safe head from L2CL via light CL mode
	sys.L2CL2.Start()

	// Wait until P2P is connected for unsafe head gossip
	sys.L2CL2.IsP2PConnected(sys.L2CL)

	// L2CL2 should catch up via light CL mode
	target = uint64(20)
	...
	logger.Info("SyncTester Light CL Mode test completed successfully",
