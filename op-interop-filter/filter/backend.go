...
func (b *Backend) tryValidateCrossUnsafe() {
	...
	oldTimestamp := b.crossUnsafeTimestamp.Swap(minTimestamp)
	if minTimestamp != oldTimestamp {
		b.log.Debug("Cross-unsafe timestamp advanced", "from", oldTimestamp, "to", minTimestamp)
	}
