...
func TestRepairAcceptedState_RewindsToNewestConsistentTimestamp(t *testing.T) {
	...
	require.Equal(t, eth.BlockRef{}, h.Mock(10).rewindCalls[0].invalidatedBlock)
}

func TestRepairAcceptedState_IgnoresRepairResetSentinel(t *testing.T) {
	h := newInteropTestHarness(t).
		WithChain(10, nil).
		Build()
	...
	err := h.interop.applyReset(pendingReset{
		chainID:          chainID,
		timestamp:        1000,
		invalidatedBlock: eth.BlockRef{},
	})
	require.NoError(t, err)
	require.Empty(t, logsDB.rewindCalls)
	require.Zero(t, logsDB.clearCalls)

	h.interop.mu.RLock()
	require.Equal(t, eth.BlockID{}, h.interop.currentL1)
	require.False(t, h.interop.validated.Valid)
	h.interop.mu.RUnlock()
}
...
func (m *mockChainContainer) RewindEngine(ctx context.Context, timestamp uint64, invalidatedBlock eth.BlockRef) error {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.rewindCalls = append(m.rewindCalls, rewindEngineCall{timestamp: timestamp, invalidatedBlock: invalidatedBlock})
	return m.rewindErr
}
...
func (m *mockChainContainer) PruneDenyListAfter(timestamp uint64) (bool, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func TestReset(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "queues reset without applying immediately",
			setup: func(h *interopTestHarness) (*interopTestHarness, *mockLogsDBForInterop) {
			...
			run: func(t *testing.T, h *interopTestHarness, mockLogsDB *mockLogsDBForInterop) {
				...
				require.Equal(t, eth.BlockID{Number: 500, Hash: common.HexToHash("0xL1")}, h.interop.currentL1)
				require.Len(t, mockLogsDB.rewindCalls, 0)
				require.Equal(t, 0, mockLogsDB.clearCalls)
				require.Len(t, h.interop.pending, 1)
			},
		},
		{
			...
			run: func(t *testing.T, h *interopTestHarness, mockLogsDB *mockLogsDBForInterop) {
				...
				invalidatedBlock := eth.BlockRef{Number: 100, ParentHash: common.HexToHash("0xPARENT")}
				h.interop.Reset(h.Mock(10).id, 100, invalidatedBlock)
				applied, err := h.interop.applyPendingResets()
				require.NoError(t, err)
