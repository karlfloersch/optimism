...
import (
	...
	sn "github.com/ethereum-optimism/optimism/op-supernode/super"
	"github.com/ethereum-optimism/optimism/op-supernode/super/chain"
)

// Config captures CLI-derived configuration for the supernode service lifecycle.
type Config struct {
	HTTPAddr    string
	HTTPPort    int
	ProxyOpNode bool
...
func NewConfig(ctx *cli.Context, logger log.Logger) (*Config, error) {
	...
	cfg := &Config{
		...
		DataDir:      ctx.String("supernode.data-dir"),
		DisableP2P:   ctx.Bool("disable.p2p"),
		ConfigPath:   ctx.String("supernode.config"),
...
// New constructs the lifecycle for the supernode using the provided config and logger.
// It mirrors the op-node pattern: the returned lifecycle starts HTTP and the super, and stops them gracefully.
func New(_ context.Context, cfg *Config, logger log.Logger, version string, _ any) (cliapp.Lifecycle, error) {
	if cfg == nil {
		return nil, fmt.Errorf("nil config")
	}
	// Build a super instance
	sup := sn.NewSuper(logger)
	if cfg.DataDir != "" {
	...
	if strings.TrimSpace(cfg.ConfigPath) != "" {
		...
		type fileCfg struct {
			...
			SupernodeDataDir string     `json:"supernode_data_dir"`
		...
		if err != nil {
			return nil, fmt.Errorf("read supernode.config: %w", err)
		}
		var fc fileCfg
		if err := json.Unmarshal(raw, &fc); err != nil {
			return nil, fmt.Errorf("parse supernode.config: %w", err)
		}
		...
		if fc.SupernodeDataDir != "" {
			cfg.DataDir = fc.SupernodeDataDir
		}
		...
		for i, c := range fc.Chains {
			...
			go func() {
				...
				if c.L1RPC == "" || c.BeaconAddr == "" || c.L2AuthRPC == "" || c.L2UserRPC == "" || c.JWTPath == "" || c.RollupPath == "" {
					errCh <- fmt.Errorf("supernode.config chains[%d]: missing required fields", i)
					return
				}
				// Read JWT
	...
	// Return a lifecycle that manages HTTP and underlying super shutdown
	return &supernodeLifecycle{srv: httpSrv, sup: sup, logger: logger, version: version, addr: httpAddr}, nil
}

type supernodeLifecycle struct {
	srv     *http.Server
	sup     *sn.Super
	logger  log.Logger
	version string
...
func (l *supernodeLifecycle) Start(ctx context.Context) error {
	...
	go func() {
		l.logger.Info("starting supernode http server", "addr", ln.Addr().String(), "version", l.version)
...
func (l *supernodeLifecycle) Stop(ctx context.Context) error {
	if l.stopped {
		return nil
	}
	l.stopped = true
...
func (l *supernodeLifecycle) Stopped() bool { return l.stopped }

// AddChain registers a chain on a running supernode lifecycle instance. Intended for tests/harnesses.
// It avoids importing internal cmd or super wiring from callers.
func AddChain(lc cliapp.Lifecycle, vCfg *chain.VirtualNodeConfig) (uint64, error) {
	s, ok := lc.(*supernodeLifecycle)
...
// RollbackChain requests a rollback of a chain managed by the lifecycle's super.
