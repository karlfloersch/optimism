...
func (s *Super) getCurrentCrossSafeTimestamp() uint64 {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *Super) getLatestCrossSafe() *crossSafeMD {
	s.mu.Lock()
	defer s.mu.Unlock()
	if len(s.crossSafeHistory) == 0 {
		return nil
...
func (s *Super) addCrossSafeEntry(entry crossSafeMD) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *Super) setCrossSafeTimestamp(timestamp uint64) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *Super) pruneLatestCrossSafeEntry() uint64 {
	s.mu.Lock()
	defer s.mu.Unlock()
	if len(s.crossSafeHistory) == 0 {
		return 0
...
func (s *Super) loadCrossSafeHistory() error {
...
func (s *Super) saveCrossSafeHistory() error {
	if s.crossSafeHistoryFile == "" {
		return nil // no file configured
	}
...
func (s *Super) pruneContainersToTimestamp(ctx context.Context, activeChains []eth.ChainID, targetTimestamp uint64) {
...
func (s *Super) ProgressCrossSafe() {
...
func (s *Super) checkL1Consistency(ctx context.Context, activeChains []eth.ChainID) bool {
...
func (s *Super) initializeCrossSafeTimestamp(activeChains []eth.ChainID) bool {
...
func (s *Super) computeCandidateTimestamp() uint64 {
...
func (s *Super) validateExecutingMessagesAtTimestamp(ctx context.Context, activeChains []eth.ChainID, ts uint64) bool {
...
func (s *Super) commitNewTimestamp(ts uint64, pairs map[uint64]types.DerivedBlockRefPair) {
...
func (s *Super) ingestRange(ctx context.Context, l2 *sources.L2Client, logs *logsdb.DB, start, end uint64) error {
	for n := start; n <= end; n++ {
...
func (s *Super) xsafeSnapshotActiveChains() []eth.ChainID {
	s.chainsMu.Lock()
...
func (s *Super) getMinGenesisTimestamp(activeChains []eth.ChainID) uint64 {
...
func (s *Super) getBlocksAtTimestamp(ctx context.Context, activeChains []eth.ChainID, ts uint64) (map[uint64]types.DerivedBlockRefPair, error) {
...
func (s *Super) xsafeIngestLogsTo(ctx context.Context, activeChains []eth.ChainID, pairs map[uint64]types.DerivedBlockRefPair) bool {
	ready := true
...
func (s *Super) getExecutingMessages(activeChains []eth.ChainID, ts uint64) map[uint64]map[uint32]*types.ExecutingMessage {
...
func (s *Super) validateExecutingMessages(ctx context.Context, activeChains []eth.ChainID, ts uint64, execByChain map[uint64]map[uint32]*types.ExecutingMessage) bool {
	allValid := true
	invalidCount := 0
	totalCount := 0
...
func (s *Super) authorizeFinalityUpdate(timestamp uint64) bool {
	s.mu.Lock()
	defer s.mu.Unlock()
