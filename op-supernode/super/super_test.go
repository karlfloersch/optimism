...
func TestCrossSafeHistoryPersistence(t *testing.T) {
	...
	tempDir, err := os.MkdirTemp("", "super-test-*")
	if err != nil {
		t.Fatalf("Failed to create temp dir: %v", err)
	}
	defer os.RemoveAll(tempDir)

	// Create super and set data dir
	logger := log.New()
	s := NewSuper(logger)
	...
	// Create a new super with the same data dir to test loading
	s2 := NewSuper(logger)
	...
	// Create a third super to verify pruning was persisted
	s3 := NewSuper(logger)
