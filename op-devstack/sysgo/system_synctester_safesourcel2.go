...
type DefaultSimpleSystemWithSyncTesterLightModeIDs struct {
...
func NewDefaultSimpleSystemWithSyncTesterLightModeIDs(l1ID, l2ID eth.ChainID) DefaultSimpleSystemWithSyncTesterLightModeIDs {
	minimal := NewDefaultMinimalSystemIDs(l1ID, l2ID)
	return DefaultSimpleSystemWithSyncTesterLightModeIDs{
...
func DefaultSimpleSystemWithSyncTesterLightMode(dest *DefaultSimpleSystemWithSyncTesterLightModeIDs) stack.Option[*Orchestrator] {
	l1ID := eth.ChainIDFromUInt64(900)
	l2ID := eth.ChainIDFromUInt64(901)
	ids := NewDefaultSimpleSystemWithSyncTesterLightModeIDs(l1ID, l2ID)

	opt := stack.Combine[*Orchestrator]()
	opt.Add(stack.BeforeDeploy(func(o *Orchestrator) {
		o.P().Logger().Info("Setting up light CL mode follower")
	}))
	...
	// Create L2CL2 with light CL mode pointing to L2CL and setup P2P connection
	...
	opt.Add(stack.AfterDeploy(func(orch *Orchestrator) {
		...
		require.True(ok, "L2EL node must exist before creating L2CL2 with light mode")

		// Create L2CL2 with light CL mode configuration
		...
		nodeOpt := WithL2CLNode(ids.L2CL2, ids.L1CL, ids.L1EL, ids.SyncTesterL2EL, lightModeOpt)
		nodeOpt.Deploy(orch)
